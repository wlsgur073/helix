...
- `egressPolicy` — the per-leg map the egress guard consults, one key per class of content it can
  ...
  shape (dated paths, note slugs, citations — no credential keyword in the same statement) persists
  verbatim under its own key, `persistence.releaseWordChains` (default `true`); a hex-core token
