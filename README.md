...
- **Scale-trigger snapshot** (`node bin/helix-trigger.mjs --root <path> --run <id>`; run by you or your own scheduler, never launched by the plugin or by Claude Code): appends one content-free evaluation of the indexed-storage migration trigger (ledger row / byte / latency legs) to `~/.helix/trigger.jsonl` — the measurement behind the Scale note in [Requirements](#requirements). Both arguments are required: `--root` is the project root the evaluation is about, `--run` is an id you choose to label the record. Run as `node bin/helix-trigger.mjs --acknowledge` instead, it records that the latest evaluation's fire was handled: it appends one content-free acknowledgement to the same file, copying that evaluation's leg readings, and refuses with exit 2, appending nothing, when the latest evaluation does not read `fired`. Invoked with neither `--root`/`--run` nor `--acknowledge`, the command prints its usage and exits 2.
