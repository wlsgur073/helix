...
- **A reclaimed lock is proved dead on Linux in every case, and on Windows across a reboot; on macOS
  it is still only waited out.** A holder records its pid together with the process start time read
  from `/proc`, and a waiter reclaims the lock when that recorded start time differs from the one the
  pid carries now — positive proof the original process is gone. Platforms without `/proc` (macOS,
