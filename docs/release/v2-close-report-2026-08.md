...
- The **second machine's** pins — the one deployment surface this report cannot re-derive from the measuring box: NOT OBSERVED — the abort ended the window before A1 ran; the second machine's pins were never observed and cannot be reconstructed after the fact. The v3.4 script/doc pair was staged for transfer on 2026-08-31 (SHA256SUMS verified); it remains runnable as an ordinary post-abort check, but no §4 pin report exists. **DISPOSITION 2026-09-01 (owner-approved):** as a protocol obligation this item is CANCELED, NOT PENDING — the abort voided the close and its prerequisites, so no session should re-execute the ledger's former "must run before the close" instruction on this window's authority; the pair stays on disk as a historical artifact, and any future window must issue its own requirement
...
- Owner-owed operational items, **reported open if unexecuted**: Q1 recovery codes: OPEN, not executed; Q2 credential inventory: OPEN, not executed; Q4 encrypted backup: PRODUCED AND VERIFIED 2026-08-31 23:00 KST after one failed attempt at 22:11 (pinentry could not reach the terminal; the loopback prompt resolved it) — 101,632 bytes, AES256 symmetric, archive sha256 `8171a536fba3792e0e7996ad06165dcd2565ea0c1ff6867c8affa88eedeb5bd6`, `ledger-mac-master.key` listed with mode `-rw-------` under decryption, no plaintext archive anywhere; the separate-medium copy and the off-box passphrase note remain OPEN, the owner's physical acts. **UPDATE 2026-09-01:** Q1 CLOSED — the owner confirmed recovery codes are issued and held; Q2 CLOSED — the owner confirmed this box is the only working machine and every session was first-person (single-operator risk accepted and recorded); Q4's separate-medium copy EXISTS and was sha256-verified from this box against the archive digest above, and the passphrase is held on paper off this box — the medium's physical detachment is the one remaining open act. **CORRECTION 2026-09-03, recording a measurement made 2026-09-02 and reproduced 2026-09-03 rather than an inference:** that sentence claims more than was verified. The directory that received the copy is not a mounted medium — its device id equals the root filesystem's, only the two fixed Windows drives are drvfs-mounted, and the operator log for that day carries the directory creation and the copy but no `mount` command, so the path was created as an ordinary directory on the system disk. The sha256 comparison did run and did match; what it matched was a second copy on the SAME physical disk as its source. Q4's separate-medium requirement is therefore NOT met — not merely undetached — and the copy has to be made again onto a genuinely mounted medium, verified there, and then detached. The same applies to the two other retention artifacts placed in that directory the same day, `helix-chain-2026-08-31.tar.gz` (291,901 B) and `helix-snapshot-2026-08-31.tar.gz.gpg` (51,533 B): both carry the same device id as the system disk, so the durable off-machine copy this record and `v2-close-evidence-index-2026-08.md` both call PENDING is still pending, and that directory must not be used to discharge it. The passphrase-on-paper half is unaffected and stands
...
## Appendix A — the close-run evidence index (moved here 2026-09-09)

`v2-close-evidence-index-2026-08.md` was removed from this directory on 2026-09-09 in the
pre-release cleanup. It held one artifact table and one open item, and both are reproduced below
verbatim so the deletion destroys nothing. Its own bytes remain recoverable:
`git log --diff-filter=D --oneline -- docs/release/v2-close-evidence-index-2026-08.md`.
...
**Its open item, still open on 2026-09-09.** The durable second copy of the non-secret evidence
chain has not been made. It is tracked from here on in `README.md` §4 of this directory, beside the
C4.6-Q4 separate-medium copy it must not be discharged against.

**Two pointer corrections, recorded rather than edited into the text above.** The index says the
identity walk is "§8 of the report"; the walk is §7.7 — `## 8.` is the O_67 census. The same
off-by-one appears where the blocked verdict is cited to §8; it is in §9.

## Appendix B — what moved after this record was written (2026-09-09)
...
- **The freeze guard.** `scripts/freeze-runtime-check.sh` was retired in place on 2026-09-05 and
  **deleted** on 2026-09-09 together with `v2-freeze-runtime-pins-2026-08.txt`, so the D1 and G4
  ...
  is a different program — `scripts/freeze-guard.ts` — and it still ships, still runs in CI, and
  still verifies this window's anchors.
- **The window's consequence.** Ruled on 2026-09-09 in `readiness-criteria-2026-07.md` §14: the
  block attaches to the recall-quality claim, not to the product.
