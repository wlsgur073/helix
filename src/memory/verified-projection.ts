...
export function buildVerifiedProjection(
  ...
  for (const [target, verifies] of byTarget) {
    ...
    // A duplicate id ADVISES, it does not demote — unlike the equal-generation conflict below, which
    // does clamp. The difference is what the evidence supports. Two verifies disagreeing about one
    // target are BOTH signed, so the ledger genuinely cannot say which grade is right and serving
    // either would be a guess. A duplicate fact id is not that: `live` here holds the row positional
    // ownership resolved (projection.ts), the first to bear the id, and its elevation was earned
    // honestly. Demoting it would punish the victim for the forger's append — and since appending is
    // all it takes to plant a twin, demote-on-sight would hand any writer a demote-anything
    // primitive. The twin is already inert; this only records that it was seen. The evidence itself
    // is kept across rewrites by planCompaction, so the advisory outlives a compaction.
