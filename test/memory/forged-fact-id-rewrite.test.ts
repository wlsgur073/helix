...
describe('duplicate fact id survives a ledger rewrite', () => {
  ...
  it('a forged provenance twin stays compromised after a permanent erase of an UNRELATED fact', () => {
    ...
    expect(before.live.get(target.id)!.provenance.source).toBe('user');  // the OWNING row is served...
    expect(before.live.get(target.id)!.state).toBe('Verified');          // ...and keeps what it earned
    ...
    expect(after.live.get(target.id)!.provenance.source).toBe('user');  // still the owning row...
    expect(after.live.get(target.id)!.state).toBe('Verified');          // ...the twin never rode the grade
  });
  ...
  it('a twin differing ONLY in tx stays compromised after the rewrite (no field may be normalized)', () => {
    ...
    expect(before.live.get(target.id)!.state).toBe('Verified'); // flagged, not demoted
    ...
    expect(after.live.get(target.id)!.state).toBe('Verified');  // never re-graded off the surviving twin
  });
  ...
  it('holds in the ADOPTED PROJECT scope against a prefix-preserving append, across the witness advance', () => {
    ...
    expect(after.compromised.has(target.id)).toBe(true);
    expect(after.live.get(target.id)!.state).toBe('Verified');
  });
