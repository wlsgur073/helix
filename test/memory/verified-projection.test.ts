...
describe('buildVerifiedProjection', () => {
  ...
  it('a duplicate fact id is tamper evidence: flagged, while the OWNING row keeps its earned grade', () => {
    ...
    // Two separate guarantees, and this pins both: ownership (projection.ts) decides WHICH row is
    // served — the first to bear the id, never the appended one — and the duplicate flag only
    // ADVISES. It must not demote: appending is all it takes to plant a twin, so demote-on-sight
    // would be a demote-anything primitive against any fact.
    const recs = [
    ...
    expect(p.live.get('a')!.provenance.source).toBe('agent-inference'); // the FIRST row owns the id...
    expect(p.live.get('a')!.state).toBe('Corroborated');                // ...and keeps what it earned
    expect(p.compromised.has('a')).toBe(true);                          // the twin is recorded, not served
  });
