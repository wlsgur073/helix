...
describe('handleCodexStatus', () => {
  ...
  it('content log ON reports ON + path + entry count (counts existing lines, missing -> 0)', async () => {
    ...
    const onCfg: HelixConfig = { dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: null, effort: null, timeoutMs: 120_000, egressPolicy: { memoryEcho: 'block', piiHigh: 'block', piiBulk: 'block', secretHeuristic: 'block', secretEntropy: 'block', secretEntropyExempt: 'allow' }, logContent: true }, metrics: { enabled: true } };
