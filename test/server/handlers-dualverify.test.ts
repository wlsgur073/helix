...
function deps(over: Partial<DualVerifyHandlerDeps>): DualVerifyHandlerDeps {
  ...
  return {
    config: { dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: 'gpt-5.5', effort: 'high', timeoutMs: 120_000, memoryEgress: 'block', logContent: false } },
...
describe('handleDualVerify', () => {
  ...
  it('critique mode renders a DATA-framed critique block and audit-logs the mode', async () => {
    const d = deps({
      config: { dualVerify: { enabled: true, mode: 'critique', stakesFloor: 'high', model: null, effort: null, timeoutMs: 120_000, memoryEgress: 'block', logContent: false } },
...
describe('handleDualVerify egress audit', () => {
  ...
  it('logs an allowed_override when policy=allow (the highest-interest event is visible)', async () => {
    ...
    const d = deps({
      config: { dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: 'gpt-5.5', effort: 'high', timeoutMs: 120_000, memoryEgress: 'allow', logContent: false } },
...
const onConfig = (over: Partial<HelixConfig['dualVerify']> = {}) =>
  ({ dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: null, effort: null, timeoutMs: 120_000, memoryEgress: 'block', logContent: true, ...over } } as HelixConfig);
