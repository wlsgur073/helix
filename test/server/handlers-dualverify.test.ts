...
describe('handleDualVerify', () => {
  ...
  it('critique mode renders a DATA-framed critique block and audit-logs the mode', async () => {
    const d = deps({
      config: { dualVerify: { enabled: true, mode: 'critique', stakesFloor: 'high', model: null, effort: null, timeoutMs: 120_000, egressPolicy: { memoryEcho: 'block', piiHigh: 'block', piiBulk: 'block', secretHeuristic: 'block', secretEntropy: 'block', secretEntropyExempt: 'allow' }, logContent: false }, persistence: { releaseWordChains: true }, metrics: { enabled: true } },
  ...
  it('a figure-clamped indeterminate never claims the aligner found no pairs (H1)', async () => {
    ...
    const res = await handleDualVerify({ stakes: 'high', question: 'q', helixAnswer: 'The retry limit is 3.' }, d);
    const t = text(res);
    expect(t).toContain('verdict: indeterminate (mode: compare)');
  ...
  it('an agreement that SURVIVES beside a withheld pair still reaches the caller (H1 fix round 1)', async () => {
    ...
    const res = await handleDualVerify(
      { stakes: 'high', question: 'q', helixAnswer: 'The lock is safe. The retry limit is 3.' }, d);
    const t = text(res);
    expect(t).toContain('verdict: indeterminate (mode: compare)');
...
describe('handleDualVerify egress audit', () => {
  ...
  it('logs an allowed_override when policy=allow (the highest-interest event is visible)', async () => {
    ...
    const d = deps({
      config: { dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: 'gpt-5.5', effort: 'high', timeoutMs: 120_000, egressPolicy: { memoryEcho: 'allow', piiHigh: 'block', piiBulk: 'block', secretHeuristic: 'block', secretEntropy: 'block', secretEntropyExempt: 'allow' }, logContent: false }, persistence: { releaseWordChains: true }, metrics: { enabled: true } },
...
const onConfig = (over: Partial<HelixConfig['dualVerify']> = {}) =>
  ({ dualVerify: { enabled: true, mode: 'compare', stakesFloor: 'high', model: null, effort: null, timeoutMs: 120_000, egressPolicy: { memoryEcho: 'block', piiHigh: 'block', piiBulk: 'block', secretHeuristic: 'block', secretEntropy: 'block', secretEntropyExempt: 'allow' }, logContent: true, ...over } } as HelixConfig);
